    ...
    <ul className="quiz-path-list-option">
      {quizData.map((quiz, index) => {
        ...
        const totalPoint = useMemo(() => {
          let mainPoint = reportItem?.userTotalPoins;
      
          if(reportItem.metaPoints.length) {
            mainPoint = reportItem.metaPoints.reduce((total, meta) => {
        ...
        return <li className="quiz-path-list-item" key={ id } onClick={ e => onSelect(quiz) }>
          <div className="quiz-path-list-path-name">
            ...
            {
              reportItem.status == 'done' ? <span className="__status">Đã hoàn thành (+{ totalPoint } điểm)</span> : <span className="__icon"><CirclePlay /></span>
            }
